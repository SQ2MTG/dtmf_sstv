...
LONG_PRESS_THRESHOLD = 1.5  # PL: prog dlugiego nacisniecia EN: long press threshold
DEBOUNCE_MS = 50  # PL: opoznienie na drgania stykow EN: debounce delay in ms

# PL: Konfiguracja GPIO
# EN: GPIO setup
GPIO.setmode(GPIO.BCM)
GPIO.setup(PIN_D0, GPIO.IN)
GPIO.setup(PIN_D1, GPIO.IN)
GPIO.setup(PIN_D2, GPIO.IN)
GPIO.setup(PIN_D3, GPIO.IN)
GPIO.setup(PIN_STROBE, GPIO.IN)
GPIO.setup(PTT_PIN, GPIO.OUT)
GPIO.output(PTT_PIN, GPIO.LOW)

# PL: Mapowanie kodow BCD na klawisze DTMF
# EN: Map BCD codes to DTMF keys
DTMF_BCD_TO_KEY = {
    0x1: '1',
    0x2: '2',
    0x3: '3',
...
# PL: Odtwarzanie plikow dzwiekowych
# EN: Play audio files
def play_audio(file_path):
    """Play an audio file (supports both WAV and MP3)"""
    if file_path.lower().endswith('.mp3'):
        cmd = ["mpg123", file_path]
    else:  # assume WAV for other files
        cmd = ["aplay", file_path]
    try:
        subprocess.run(cmd, check=True)
        return True
    except Exception as e:
        print(f"Blad odtwarzania {file_path}:", e)
        return False
...
def send_control_image(cmd_key):
    ...
    # Odtwórz powiadomienie o generowaniu
    play_audio("trwagenerowanie.mp3")
    # Odtwórz powiadomienie o trybie (domyślnie ROBOT36 dla obrazów kontrolnych)
    play_audio("ROBOT36.mp3")
            ...
            if duration >= LONG_PRESS_THRESHOLD:
                ...
                if sstv_mode:
                    ...
                    if capture_image_usb(img_path):
                        # Odtwórz powiadomienie o generowaniu
                        play_audio("trwagenerowanie.mp3")
                        # Odtwórz powiadomienie o trybie
                        play_audio(f"{sstv_mode}.mp3")
